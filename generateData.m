...
files_emotion = dir([dir_data,'/Emotion/']);
formatSpec = '%f';
...
%     disp(['Extracting from ',files_emotion(i).name]);
    s_i = dir([dir_data,'/Emotion/',files_emotion(i).name,'/']);
    
    for j = 3:size(s_i)    
        % Looking for different emotion faces in each person's images
        disp(['      ',s_i(j).name]);
        s_i_j = dir([dir_data,'/Emotion/',files_emotion(i).name,'/',s_i(j).name]);
        
        if size(s_i_j,1) > 2
%             disp(['Extractig Emotions',files_emotion(i).name]);
            % If emotion label exists, extract emotion label from image
            textfile_emotion = fopen([dir_data,'/Emotion/',files_emotion(i).name,'/',s_i(j).name,'/',s_i_j(3).name],'r');
            labels = int32(fscanf(textfile_emotion,formatSpec));
            fclose(textfile_emotion);
             
            % Extract peak image for data
            image_dir = dir([dir_data,'/cohn-kanade-images/',files_emotion(i).name,'/',s_i(j).name]);
            peak_image_name = [dir_data,'/cohn-kanade-images/',files_emotion(i).name,'/',s_i(j).name,'/',image_dir(size(image_dir,1)).name];
...
figure;
imshow(reshape(data(:,200),[64 64]),[]);

save('DataFile','data','labels');
